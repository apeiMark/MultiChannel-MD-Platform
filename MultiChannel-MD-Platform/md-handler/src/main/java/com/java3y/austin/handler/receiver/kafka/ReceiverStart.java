...
public class ReceiverStart {
    ...
    public static KafkaListenerAnnotationBeanPostProcessor.AnnotationEnhancer groupIdEnhancer() {
        /**
        *
         * attrs: 注解属性键值对，例如@KafkaListener 中的groupId、topics属性都可以获取
         * element： 被注解标记的元素（可以是方法或者类）
        */
        return (attrs, element) -> {
            if (element instanceof Method) {
                ...
                *
                 * 如果被注解标注的为方法，则进入
                 * 并获取到被标注的方法的名称，((Method) element).getDeclaringClass().getSimpleName()这个获取到的就是被标注方法的类名称：Receiver
                 * ((Method) element).getName()获取到方法名：consumer
                 * 最终name为：Receiver.consumer与常量RECEIVER_METHOD_NAME比对成功
                 * 这里的步骤就是为了找到我们想要修改的方法的注解进行配置修改
                 *
                */
